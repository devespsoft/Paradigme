...
export default class product extends Component {
        ...
        this.columns = [
            ...
            },
            {
                ...
                cell: (item) => {
                    return (
                        <>
                            {item.nft_type_select === 0 ? 'Digital' : 'Physical'}
                        </>
                    );
                }
            },
            ...
            {
                text: "Physical Nft Verification",
                cell: (item) => {
                    return (
                        <>

                            {item.nft_type_select === 1 && item.is_physical_approved == 0 ?
                                <p style={{ color: 'red', cursor: 'pointer' }} onClick={this.verifyPhysicalNFT.bind(this, item)}>Verify</p> : item.nft_type_select === 1 && item.is_physical_approved == 1 ?
                                    <p style={{ color: 'green' }}>Verified</p> : 'NA'
                            }
                        </>
                    );
                }
            ...
            axios({
                ...
                .then(result => {
                    ...
                    if (result.data.success === true) {
                        ...
                        toast.success(result.data.msg, setTimeout(() => {
                            window.location.reload();
                        }, 1500));
                    ...
                    onClick: () =>
                        axios({
                            ...
                            .then(result => {
                                ...
                                toast.warning(err.response.data?.msg, setTimeout(() => {
    ...
    verifyPhysicalNFT(id) {
        confirmAlert({
            title: 'Confirm to submit',
            message: 'Are you sure to Approve this Physical NFT..',
            buttons: [
                {
                    label: 'Yes',
                    onClick: () =>
                        axios({
                            method: 'post',
                            url: `${config.apiUrl}verifyPhysicalNft`,
                            headers: { "Authorization": this.loginData?.Token },
                            data: { 'item_id': id.id, 'email': this.loginData?.data.user_email, is_physical_approved: 1 }
                        }).then(result => {
                            toast.success(result.data.msg);
                            this.componentDidMount();
                        }).catch((error) => {
