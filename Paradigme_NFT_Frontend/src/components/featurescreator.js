...
export default class featurescreator extends Component {
   ...
   validate = () => {
      ...
      if (this.state.quantity === '0') {
         quantityError = "Quantity is greater than 0"
      }
      if (this.state.edition_type === '') {
         editionTypeError = "Edition Type is rquired"
      }

      if (nameError || categoryNameError || collectionNameError || sellTypeError || priceError || descriptionError || imageError || expiry_dateError || quantityError || editionTypeError || greaterDate) {
         this.setState({
            nameError, categoryNameError, collectionNameError, sellTypeError, priceError, descriptionError, imageError, expiry_dateError, quantityError, editionTypeError, greaterDate
         })
         return false
      }
      return true
   ...
   async getcategoryAPI() {
      await axios({
         method: 'get',
         url: `${config.apiUrl}getUserDigitalCategory`,
         headers: { "Authorization": this.loginData.Token },
      }).then(response => {
   ...
   async paymentStripeShow() {
      ...
      await axios({
         ...
         headers: { "Authorization": this.loginData?.Token },
         data: {
            "email": this.loginData.data.user_email, "user_id": this.loginData.data.id, "amount": this.state.getListUser?.transfer_fee_usd, "number": this.state.number, "expMonth": this.state.expMonth,
                              ...
                              <div className="row">
                                 ...
                                 <div className="input-col col-xs-12 col-sm-6 image-uploaddata">
                                    <div className="form-group focus-2">
                                       ...
                                       <br />
                                       <input className="form-input" type="file" accept=".jpg,.jpeg,.png,.gif,.mp3,.mp4" onChange={this.handleImagePreview} style={{ marginTop: '10px' }} />
                                 ...
                                 <div className="input-col col-xs-6 col-sm-6">
                                    <div className="form-group focus-2">
                                       ...
                                       <input className="form-input" type="text" value={this.state.price} name="price" placeholder="Fixed price"
                                 ...
                                 <div className="input-col col-xs-6 col-sm-6">
                                    <div className="form-group focus-2">
                                       ...
                                       {this.state.edition_type === '2' ?
                                          ...
                                          :
                                          <>
                                             <input type="text" className="form-control" value={this.state.quantity} name="quantity"
                                                onChange={this.onChange} onKeyPress={(event) => {
                                                   if (!/^\d*[.]?\d{0,1}$/.test(event.key)) {
                                                      event.preventDefault();
                                                   }
