...
export default class featurescreatorrealestate extends Component {
   ...
   async onDragEnd(result) {
      ...
      console.log(this.state.mainArrPre);
      setTimeout(() => {
   ...
   validate = () => {
      ...
      if (this.state.start_date) {
         if (this.state.expiry_date <= this.state.start_date) {
            greaterDate = "Start date must be less than expiry date"
         }
         else {
      ...
      if (this.state.price === '0') {
         priceError = "Price is greater than 0"
      }
      ...
      if (nameError || categoryNameError || collectionNameError || sellTypeError || priceError || descriptionError || imageError || imageErrorMain || expiry_dateError || image_filetitle_deedError || image_filepassportError || greaterDate) {
         this.setState({
            nameError, categoryNameError, collectionNameError, sellTypeError, priceError, descriptionError, imageError, imageErrorMain, expiry_dateError, image_filetitle_deedError, image_filepassportError, greaterDate
         })
         return false
      }
      return true
   ...
   async paymentNetsents() {
      ...
      await axios({
         ...
         headers: { "Authorization": this.loginData?.Token },
         // 'amount': this.state.getListUser?.transfer_fee_usd
   ...
   async paymentStripeShow() {
      ...
      await axios({
         method: 'post',
         url: `${config.apiUrl}circleMintPayment`,
         headers: { "Authorization": this.loginData?.Token },
         data: {
            "email": this.loginData.data.user_email,
            "user_id": this.loginData.data.id, "amount": this.state.getListUser?.transfer_fee_usd, "number": this.state.number, "expMonth": this.state.expMonth,
            "expYear": this.state.expYear, "cvv": this.state.cvv
         }
      })
         .then(result => {
            if (result.data.success === true) {
   ...
   async getcategoryAPI() {
      await axios({
         method: 'get',
         url: `${config.apiUrl}getUserRealEstateCategory`,
         headers: { "Authorization": this.loginData.Token },
      }).then(response => {
         if (response.data.success === true) {
            this.setState({
   ...
   handleImagePreview = (e) => {
      ...
      console.log(this.state.firstTimeImgPre)
      console.log(this.state.firstTimeImgList);
      setTimeout(() => {
   ...
   async afterPaypalSuccess(paypalId) {
      ...
      await axios({
         method: 'post',
         url: `${config.apiUrl}paypalMintPayment`,
         headers: { "Authorization": this.loginData?.Token },
         data: {
            "email": this.loginData.data.user_email,
            "user_id": this.loginData.data.id,
         ...
         <>
            ...
            <div id="content-block">
               ...
               <div className="container-fluid custom-container pl-0 pr-0">
                  ...
                  {this.state.mainLoaderShow === true ?
                     <div className="row _post-container_" style={{ height: '200px', marginBottom: '200px' }}>
                        <div className="caroselHeight loaderBars">
                           <Loader type="Bars" color="#00BFFF" height={40} width={40} />
                           <h4 style={{ color: '#fff', textAlign: 'center', marginLeft: '-151px' }}>Please do not refresh page until creating NFT...</h4>
                        </div>
                     </div> :
                                             ...
                                             <div className="row">
                                                ...
                                                <div className="input-col col-xs-6 col-sm-6">
                                                   <div className="form-group focus-2">
                                                      ...
                                                      <input className="form-input" type="text" value={this.state.price} name="price" placeholder="Price"
                                                         onChange={this.onChange} onKeyPress={(event) => {
                                                         ...
                                                         <div className="form-group focus-2">
                                                            ...
                                                            <div className="form-group addnftdate">
                                                               ...
                                                               <DatePicker
                                                                  ...
                                                                  name="start_date" minDate={new Date()}
                                                                  dateFormat="MM/dd/yyyy"
                                                               />
                                                               <span className="error-asterick">{this.state.greaterDate}</span>
                                                            </div>
                                                         </div>
                                                      </div>
                                    ...
                                    <div className="row">
                                       <div className="col-ml-12 col-xs-12 col-sm-12">
                                          <div className="col-12 mt-3">
                                             <p>Price : {parseFloat(this.state.getListUser?.transfer_fee_eth).toFixed(6)} ETH ~ ${parseFloat(this.state.getListUser?.transfer_fee_usd).toFixed(2)}</p>
                                          </div>
